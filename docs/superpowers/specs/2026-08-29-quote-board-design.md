...
- 日期：2026-08-29（修訂八版：收盤定格延後至 13:35——13:30-13:34 為 MIS
  同步緩衝不抓取不定格，MENU 同規則；併入七版（盤中未成交 z="-" 記 0
  顯示 --）與六版全部內容）
...
- **收盤定格**：`POST_CLOSE` 且 NVS `lastCloseDate != today` →
  - **13:30–13:34（修訂八版緩衝）**：不抓取、不定格、不重繪（畫面維持盤中
    最後一輪）→ 睡至**當日 13:35**。原因：13:30 收盤後 MIS 完成撮合與資料
    同步需數分鐘，立即定格可能缺最後一筆（2026-09-01 使用者提出並同意）。
  - **13:35 後** → 抓取一次：
...
- **MENU（GPIO2）＝立即更新（例外路徑）**：任何狀態按下 → 立即抓取渲染
  ...
  **休市日成功回應同樣套用休市結果**（`TRADING` 時段 → 睡至隔日 09:00，
  ...
  僅限 13:35 後**（修訂八版：13:30–13:34 的 MENU 只渲染，睡至 13:35 交由
  一般路徑定格）。
...
25. 13:30 醒來（未定格）→ 不抓取、畫面不變 → 13:35 醒來抓取定格（收盤最後一筆完整）。
26. 13:30–13:34 按 MENU → 渲染但不定格 → 13:35 一般路徑定格。
