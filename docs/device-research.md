...
### 實機觀測（報價看板韌體，2026-09-01~02，serial log）

| 觀測 | 現象 | 影響與緩解 |
| --- | --- | --- |
| NTP stale 首包 | `getLocalTime()` 回傳後數秒內，第二個 SNTP 回應把時鐘修正達 ±5 分鐘（首包來源時間錯誤）；證據：喚醒輪印出的 `local=09:00` 與後續 `sleep 105s`（=13:35:00-09:03:15）不一致 | 睡眠目標皆為對齊之絕對 epoch，時鐘偏差只造成該輪提前喚醒多跑一輪，下一輪自動回正；報價 header 時間取自 TWSE `t` 欄位，不受本地時鐘影響 |
| deep sleep timer 漂移 | 69893 s 的 timer 提前約 105 s 喚醒（~1500 ppm）；S3 內部 RC slow clock 無外部 32k 晶振 | 同上——絕對 epoch 目標使漂移僅造成額外喚醒輪（多一次 Wi-Fi+抓取循環），排程對齊不受影響；續航估算應把「每天額外 1-2 輪」計入 |

兩者皆為自限性行為，v1 不做程式修正；若未來要縮短 NTP 修正時間，可在
`getLocalTime()` 後延遲重查一次（`sntp_restart()` 強制重新查詢）。
...
### awake 提示條覆蓋底部預報的還原缺陷（2026-08-25 修復）

- **症狀**：awake 模式中選擇「與目前相同的地點」並下壓確認（或 idle
  逾時退出）後，畫面最底部呈現空白條，五格預報的下緣文字消失。
- **根因**：`uiClearHint()` 以白色 partial refresh 塗回提示條區域
  （y248–272），但看板五格數值文字（基線 y=254）本來就延伸進該帶域；
  塗白等於擦除內容。切換地點時隨後的全刷會重繪掩蓋此問題，
  同地點確認／逾時退出兩條路徑不做全刷，缺陷因而顯現。
  此問題自提示條功能引入即存在，非 NVS 寫入策略造成。
- **修復**：`awakeLoop()` 不再自行清除提示條，只回傳選擇結果；
  呼叫端對「同地點／逾時」路徑以 RTC 快取重繪整頁（能進 awake 代表
  初始更新已成功，快取必然有效且即為畫面現行資料，重繪像素一致）。
  `uiClearHint()` 移除。
...
## 報價看板雙頁八檔（v2）實機觀測（2026-09-04，serial log＋目視）

量測方法：報價看板 v2 韌體 serial log（分支
`feature/quote-board-multipage`，HEAD commit `f6d2667`），時間戳取自韌體
`millis()` 方括號計數（`[0000000634]` 即開機後約 634 ms）；USB-C 經 CH340C
供電（電池尚未接入）；依賴 GxEPD2 1.6.9、ArduinoJson 7.4.3、
U8g2_for_Adafruit_GFX 1.8.0，`platformio.ini` 同 `esp32eink` 主環境。
目視由使用者回報，log 行為由 serial 逐行核對。
